 ...
 * @author JQuintero
 */
public class DiaImp implements implementss {
    ...
    @Override
    public void dibujarRea(int padre, int hijo, ArrayList<Clases> clss, ArrayList<String> nombres) {
        if (clss.get(padre).creada && clss.get(hijo).creada) {
            ...
            try {
                ImageIO.write(imagen, "jpg", new File("imgs/rea" + padre + hijo + ".jpg"));
                cnt++;
                lista.add(tem);
            } catch (IOException e) {
...
interface implementss {
    ...
    void dibujarRea(int padre, int hijo, ArrayList<Clases> clss, ArrayList<String> nombres);
}
